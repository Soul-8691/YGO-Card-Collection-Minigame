"""
Generate WCT06.conf from WCT06.txt card names.

Looks up card IDs via the YGOProDeck API and ban statuses from 2005-09-01.name.json.
...
CARD_LIST_FILE = os.path.join(SCRIPT_DIR + '\\output', "victory_ocg.txt")
BAN_LIST_FILE = os.path.join(SCRIPT_DIR + '\\ban', "2005-09-01.name.json")
OUTPUT_FILE = os.path.join(SCRIPT_DIR + '\\output', "OCG-2005-09-01.conf")
...
def load_card_names(filepath):
    """Load card names from WCT06.txt, skipping blank lines and tokens."""
    names = []
...
def load_ban_list(filepath):
    """Load ban statuses from the JSON file. Returns dict of name -> status."""
    with open(filepath, "r", encoding="utf-8") as f:
        return json.load(f)


def clean_name_for_api(name):
    """Clean card name for API lookup. Some names in WCT06.txt have suffixes like '(card)'."""
...
# Some cards in WCT06.txt use names that differ from the API's canonical names.
# Map from WCT06.txt name -> API name
NAME_OVERRIDES = {
...
def fetch_card_batch(names):
    ...
    try:
        ...
        req.add_header("User-Agent", "WCT06ConfGenerator/1.0")
...
def main():
    ...
    # Deduplicate API names while preserving order, and handle special cases
    # (e.g., "Dark Magician (Arkana)" maps to same API name as "Dark Magician")
    unique_api_names = []
    seen_api = set()
        ...
        try:
            ...
            for name in batch:
                if name in results:
                    ...
                    # Try individually for names not found in batch
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        ...
        f.write("--StartDate 1970-01-01\n")
        f.write("--EndDate 2005-09-01\n")
        f.write("$whitelist\n")
