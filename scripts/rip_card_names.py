#!/usr/bin/env python3
"""Fetch Yu-Gi-Oh! card names from Format Library API (paginated) into a text file."""
...
def fetch_page(page: int, limit: int, sort: str, filter_expr: str, timeout: int) -> list[dict]:
    ...
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "Mozilla/5.0 (compatible; YGOrange-name-rip/1.0)",
...
def main() -> None:
    ...
    ap.add_argument(
        "-o",
        "--output",
        default="output/cards.txt",
        help="Output path (one card name per line)",
    )
    ap.add_argument("--pages", type=int, default=26, help="Number of pages to fetch")
    ...
    print(f"Wrote {len(names)} card names to {args.output}", file=sys.stderr)
